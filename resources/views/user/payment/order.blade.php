...
<div class="container mt-4">
    ...
    <div class="card mb-3 shadow-sm">
        ...
        <div class="card-body">
            ...
            <div class="mt-2">
                <strong>🚚 Trạng thái giao hàng GHN:</strong><br>
                <span>Mã vận đơn: <b>{{ $order->order_code }}</b></span><br>
                @if(isset($order->tracking))
                <span>Trạng thái: <b>{{ $order->tracking['status'] ?? '-' }}</b></span><br>
                <span>Người nhận: {{ $order->tracking['to_name'] ?? '-' }} - {{ $order->tracking['to_phone'] ?? '-' }}</span><br>
                <span>Địa chỉ nhận: {{ $order->tracking['to_address'] ?? '-' }}</span><br>
                <span>Ngày tạo đơn: {{ $order->tracking['created_date'] ?? '-' }}</span><br>
                <span>Ngày cập nhật: {{ $order->tracking['updated_date'] ?? '-' }}</span><br>
                <span>Ghi chú: {{ $order->tracking['note'] ?? '-' }}</span><br>
                <span>Lịch sử trạng thái:</span>
                @if(isset($order->tracking['log']) && is_array($order->tracking['log']))
                <ul>
                    @foreach($order->tracking['log'] as $item)
